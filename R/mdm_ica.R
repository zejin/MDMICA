...
#' @param mdm_type The type of mutual dependence measures, including
#' \itemize{
...
#' }
#' @param opt_algo The algorithm of optimization, including
#' \itemize{
...
mdm_ica <- function(X, num_lhs = NULL, mdm_type = "comp", num_bo = NULL, kernel = "exp", opt_algo = "par") 
{
  X <- as.matrix(X)
  num_obs <- nrow(X)
  num_comp <- ncol(X)
  ...
  if (mdm_type == "asym") {
    ...
    if (is.null(num_bo)) {
      ...
      theta_init <- bayes_opt(Z, theta_eva = theta_mat, obj_eva = obj_list, mdm_type = mdm_type, 
        num_bo = num_bo, kernel = kernel)$theta
    }
  } else if (mdm_type == "sym") {
    ...
    stop("Invalid mdm_type. Read ?mdm_ica for proper syntax.")
  }

  out <- mdm_ica_opt(Z, theta_init = theta_init, mdm_type = mdm_type, opt_algo = opt_algo)
  
  return(out)
}

# input Z should be prewhitened
mdm_ica_opt <- function(Z, theta_init = NULL, mdm_type, opt_algo) {
  d <- dim(Z)[2]
  p <- d * (d - 1) / 2
  ...
  if (opt_algo == "def") {
    ...
    stop("Invalid opt_algo. Read ?mdm_ica for proper syntax.")
  }
...
mdm_ica_def_obj <- function(Z, index, theta, mdm_type) {
  if (mdm_type == "asym") {
    ...
    stop("opt_algo = \"def\" is only for mdm_type = \"asym\".")
  }
}

mdm_ica_par_obj <- function(Z, mdm_type) {
  if (mdm_type == "asym") {
    function(p) {
      W <- theta_to_W(p)
      S <- Z %*% t(W) 
...
bayes_opt <- function(Z, theta_eva, obj_eva, mdm_type, num_bo, kernel) {
  d <- dim(Z)[2]
  p <- d * (d - 1) / 2
